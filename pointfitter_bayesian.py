...
#        mainframe.rowconfigure(0,weight=1)
    ...
    def ajuste2(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            a=pymc.Normal("a", salida[0], 0.00003)
            b=pymc.Normal("b", salida[1], 0.00003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion2var(x=x,a=a,b=b):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion2var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion2var, a, b, ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
    ...
    def ajuste3(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            a=pymc.Normal("a", salida[0], 0.00003)
            b=pymc.Normal("b", salida[1], 0.00003)
            c=pymc.Normal("c", salida[2], 0.00003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion3var(x=x,a=a,b=b,c=c):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion3var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion3var, a, b,c,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            salida[2]=np.mean((M.trace('c')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
            cov[2][2]=np.std((M.trace('c')[:]))**2
        puntos=np.linspace(xx.min(),xx.max(),1000)
    ...
    def ajuste4(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            b=pymc.Normal("b", salida[1], 0.00003)
            c=pymc.Normal("c", salida[2], 0.00003)
            d=pymc.Normal("d", salida[3], 0.00003)

            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion4var(x=x,a=a,b=b,c=c,d=d):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion4var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion4var, a, b,c,d,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            salida[2]=np.mean((M.trace('c')[:]))
            salida[3]=np.mean((M.trace('d')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
            cov[2][2]=np.std((M.trace('c')[:]))**2 
            cov[3][3]=np.std((M.trace('d')[:]))**2
    ...
    def ajuste5(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            c=pymc.Normal("c", salida[2], 0.00003)
            d=pymc.Normal("d", salida[3], 0.00003)
            f=pymc.Normal("f", salida[4], 0.00003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion5var(x=x,a=a,b=b,c=c,d=d,f=f):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion5var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion5var, a, b,c,d,f,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            salida[2]=np.mean((M.trace('c')[:]))
            salida[3]=np.mean((M.trace('d')[:]))
            salida[4]=np.mean((M.trace('f')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
            cov[2][2]=np.std((M.trace('c')[:]))**2 
            cov[4][4]=np.std((M.trace('f')[:]))**2
        puntos=np.linspace(xx.min(),xx.max(),1000)
    ...
    def ajuste6(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            a=pymc.Normal("a", salida[0], 0.000003)
            b=pymc.Normal("b", salida[1], 0.000003)
            c=pymc.Normal("c", salida[2], 0.000003)
            d=pymc.Normal("d", salida[3], 0.000003)
            f=pymc.Normal("f", salida[4], 0.000003)
            g=pymc.Normal("g", salida[5], 0.000003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion6var(x=x,a=a,b=b,c=c,d=d,f=f,g=g):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion6var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion6var, a, b,c,d,f,g,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            ...
            salida[3]=np.mean((M.trace('d')[:]))
            salida[4]=np.mean((M.trace('f')[:]))
            salida[5]=np.mean((M.trace('g')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
            cov[2][2]=np.std((M.trace('c')[:]))**2 
            cov[4][4]=np.std((M.trace('f')[:]))**2  
            cov[5][5]=np.std((M.trace('g')[:]))**2
        puntos=np.linspace(xx.min(),xx.max(),1000)
    ...
    def ajuste7(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            f=pymc.Normal("f", salida[4], 0.000003)
            g=pymc.Normal("g", salida[5], 0.000003)
            h=pymc.Normal("h", salida[6], 0.000003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion7var(x=x,a=a,b=b,c=c,d=d,f=f,g=g,h=h):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion7var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion6var, a, b,c,d,f,g,h,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
            ...
            salida[4]=np.mean((M.trace('f')[:]))
            salida[5]=np.mean((M.trace('g')[:]))
            salida[6]=np.mean((M.trace('h')[:]))
            cov[0][0]=np.std((M.trace('a')[:]))**2
            cov[1][1]=np.std((M.trace('b')[:]))**2
            cov[2][2]=np.std((M.trace('c')[:]))**2 
            cov[4][4]=np.std((M.trace('f')[:]))**2  
            cov[5][5]=np.std((M.trace('g')[:]))**2  
            cov[6][6]=np.std((M.trace('h')[:]))**2
        puntos=np.linspace(xx.min(),xx.max(),1000)
    ...
    def ajuste8(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            g=pymc.Normal("g", salida[5], 0.000003)
            h=pymc.Normal("h", salida[6], 0.000003)
            i=pymc.Normal("i", salida[7], 0.000003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion8var(x=x,a=a,b=b,c=c,d=d,f=f,g=g,h=h,i=i):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion8var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion6var, a, b,c,d,f,g,h,i,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
    ...
    def ajuste9(self,*args):
        ...
        if self.usarincertidumbres.get()==0:
            ...
            h=pymc.Normal("h", salida[6], 0.000003)
            i=pymc.Normal("i", salida[7], 0.000003)
            j=pymc.Normal("j", salida[8], 0.000003)
            x = pymc.Normal("weight", 0, 1000, value=xx, observed=True)
            funcion=self.entrada.get()
            @pymc.deterministic
            def funcion9var(x=x,a=a,b=b,c=c,d=d,f=f,g=g,h=h,i=i,j=j):
                y=eval(funcion)
                return y
            ye=pymc.Normal("y",mu=funcion7var,tau=1./sigma**2, value=yy, observed=True)
            model = pymc.Model([funcion6var, a, b,c,d,f,g,h,i,j,ye, x])
            M = MCMC(model)
            M.sample(iter=15000, burn=10000, thin=10)
            salida[0]=np.mean((M.trace('a')[:]))           
            salida[1]=np.mean((M.trace('b')[:]))
